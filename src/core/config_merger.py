...
class ConfigMerger:
    ...
    def _log(self, level: str, message: str):
        """Log a message using the logger or module-level logger."""
        if self.logger:
            getattr(self.logger, level, logger.info)(message)
        else:
                ...
                if isinstance(value, dict):
                    ...
                    if strategy == self.MERGE_STRATEGY_OVERRIDE:
                        ...
                        if remove_by and isinstance(base_value, list):
                            result[key] = [
                                item for item in base_value if item.get("description") != remove_by
                            ]
                            self.report.removed_items.append(current_path)
                            self._log(
                                "debug", f"Remove: {current_path} (by description: {remove_by})"
                            )
                        continue
    ...
    def _merge_list_by_description(
        ...
        desc_to_index = {
            item.get("description"): i for i, item in enumerate(result) if "description" in item
        }
        ...
        for extra_item in extra:
            ...
            if description and description in desc_to_index:
                ...
                idx = desc_to_index[description]
                result[idx] = self._deep_merge(result[idx], extra_item, f"{path}[{description}]")
            else:
                # New item, append
    ...
    def resolve_dependencies(self, rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for rule_id, deps in dep_graph.items():
            for dep in deps:
                if dep in in_degree:
                    ...
                    self.report.warnings.append(f"Rule '{rule_id}' depends on unknown rule '{dep}'")
        ...
        if len(result) < len(rules):
            ...
            raise ConfigMergeError(
                f"Circular dependency detected involving: {remaining_ids}", self.report
            )
